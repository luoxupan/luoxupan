...
int yg_http_parse_request_line(yg_http_request_t *r) {
    u_char ch, *p, *m;
    enum {
        sw_start = 0,
        sw_method,
    ...
    for (p = r->pos; p < r->last; p++) {
        ...
        case sw_start:
            ...
            if ((ch < 'A' || ch > 'Z') && ch != '_') {
                return YG_HTTP_PARSE_INVALID_METHOD;
            }
        ...
        case sw_method:
            if (ch == ' ') {
                ...
                case 3:
                    if (yg_str3_cmp(m, 'G', 'E', 'T', ' ')) {
                        r->method = YG_HTTP_GET;
                        break;
                    }
                ...
                case 4:
                    if (yg_str3Ocmp(m, 'P', 'O', 'S', 'T')) {
                        r->method = YG_HTTP_POST;
                        break;
                    }

                    if (yg_str4cmp(m, 'H', 'E', 'A', 'D')) {
                        r->method = YG_HTTP_HEAD;
                        break;
                    }

                    break;
                default:
                    r->method = YG_HTTP_UNKNOWN;
                    break;
                }
        ...
        case sw_spaces_before_uri:
            ...
            switch (ch) {
                case ' ':
                    break;
                default:
                    return YG_HTTP_PARSE_INVALID_REQUEST;
            }
            break;
...
int yg_http_parse_request_body(yg_http_request_t *r) {
    u_char c, ch, *p, *m;
    enum {
        sw_start = 0,
        sw_key,
    ...
    for (p = r->pos; p < r->last; p++) {
        ...
        case sw_spaces_before_colon:
            if (ch == ' ') {
                ...
                return YG_HTTP_PARSE_INVALID_HEADER;
            }
        ...
        case sw_cr:
            if (ch == LF) {
                ...
                hd = (yg_http_request_t *)malloc(sizeof(yg_http_request_t));
